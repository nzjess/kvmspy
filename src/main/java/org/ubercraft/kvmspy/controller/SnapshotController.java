...
import org.ubercraft.kvmspy.repository.DomainEntryRepository;
...
public class SnapshotController {
    ...
    @Autowired
    private DomainEntryRepository domainEntryRepository;
    ...
    public Snapshot snapshot() {
        ...
        executor.execute(() -> {
            final SnapshotSession session = new SnapshotSession(
                    snapshotRepository.findOne(snapshotId),
                    hostEntryRepository.findAllEnabled());

            Set<DomainEntry> entries = domainEntryRepository.findAllEnabled();

            entries.forEach(entry -> {
                completionService.submit(() -> {
                    PhysicalMachine machine = machineFactory.createPhysicalMachine(session, entry.getDomain());
